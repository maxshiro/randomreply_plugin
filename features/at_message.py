...
class AtMessageFeature:
    ...
    def pick(self, group_id: str, message_text: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        ...
        if reply and reply_type:
            candidates.append(((reply, reply_type, source), int(self.get_config("at_random_reply_weight", 40))))

        if bool(self.get_config("enable_repeat", False)) and message_text:
            candidates.append(((message_text, "text", "repeat"), int(self.get_config("at_repeat_weight", 20))))

        api_ref = self.pick_api_image(True)
        if api_ref:
            candidates.append(((api_ref, "photo", "api"), int(self.get_config("at_api_image_weight", 20))))

        fixed_messages = [str(x).strip() for x in self.get_config("at_fixed_messages", ["我在"]) if str(x).strip()]
        if fixed_messages:
            candidates.append(
                ((random.choice(fixed_messages), "text", "fixed"), int(self.get_config("at_fixed_message_weight", 20)))
            )
